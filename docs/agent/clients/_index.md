...
One file per client. Each brief is the single source of truth the agent should consult before any action involving that client.

| Client | Subject | GA4 property | Website | Monday board |
|---|---|---|---|---|
...
Output owner for every client is `hello@agents.digital`.

## Conventions

- "Subject" means the impersonated Google user the MCP server uses for GA4 reads.
- "Output owner" is who creates and owns generated Docs/Sheets.
- "Drive folder ID" is the parent folder under `Agents Digital / Clients`. New report files currently land in `GOOGLE_DRIVE_REPORTS_FOLDER_ID`, not the per-client folder; manual filing is a separate step.
- Monday board IDs come from the Main workspace (`2556079`) unless flagged otherwise.
